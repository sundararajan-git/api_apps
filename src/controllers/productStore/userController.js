...
} from "../../emailTemplates/productStore/emailTemplate.js";
...
export const signUp = async (req, res, next) => {
  try {
    ...
    const jwt = generateToken(res, user._id, next);
...
export const verifyEmail = async (req, res, next) => {
  try {
    ...
    await MailService.sendWelcomeCall(
      user.email,
      subject,
      user.name,
      sendWelcomeTemplate
...
export const resetPassword = async (req, res, next) => {
  try {
    ...
    await MailService.passwordRestSuccess(
      user.email,
      subject,
      resetSuccessTemplate
    );
...
export const updateProfile = async (req, res, next) => {
  try {
    ...
    const jwt = generateToken(res, res._id, next);

    res
      .status(200)
      .json({ message: "Profile updated successfully", token: jwt });
  } catch (err) {
    next(err);
  }
};
